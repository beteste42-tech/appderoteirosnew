...
## Visão Geral
- Frontend SPA com `Vite + React + TypeScript` e `Tailwind`.
- Persistência e dados via `Supabase` (tabelas: `rotas`, `rota_clientes`, `mapa_carregamento`, etc.).
- Geração de PDF com `jspdf` e captura de elementos com `html2canvas`.
...
- Typecheck: `tsconfig.dualite.json` para análise mais rígida.

## Deploy
- Netlify: `netlify.toml` (já existente).
- Vercel:
  - `vercel.json` com `@vercel/static-build` e rewrite SPA:
    ```json
    {
      "builds": [
